var wordLists = {
  "coding": ["computer", "array", "react", "string", "boolean", "function",
    "object", "method", "callback", "property", "github", "javascript",
    "jquery", "language", "programming", "syntax"
  ],
  "potter": ["hermione", "weasley", "dumbledore", "sirius", "hagrid",
    "patronus", "hogwarts", "slytherin", "ravenclaw", "hufflepuff",
    "gryffindor", "mcgonagall", "snape", "flitwick", "sprout", "longbottom",
    "lovegood", "ollivander", "voldemort"
  ],
  "lotr": ["aragorn", "frodo", "samwise", "legolas", "gimli",
    "gandalf", "peregrin", "meriadoc", "sauron", "saruman", "treebeard",
    "arwen", "boromir", "eowyn"
  ]
};
...
var WordToGuess = {
  ...
  revealLetters: function(guessedLetter) {
    var key = this.key;
    this.revealedLetters = this.revealedLetters.map(function(revealedLetter,
      index) {
...
$(function() {
  ...
  function newGame(listOfWords) {
    ...
    game.wordToGuess.revealedLetters.forEach(function(letter) {
      $('#word-container').append("<p class='letter'>" + letter +
        "</p>");
    });

    $('#unguessed-letters').empty();
    game.listOfLetters.forEach(function(letter) {
      $("#unguessed-letters").append("<span class='letter-choice'>" +
        letter + "</span>");
    });
    ...
    $(".letter-choice").click(function() {
      ...
      if (game.hasBeenGuessed(guessedLetter)) {
        ...
        alert(
          "That letter has already been guessed. Please select another letter."
        );
      } else {
        ...
        $('div#hangman-sprite').removeClass().addClass('hangman-sprite' +
          game.hangman.bodyParts);
  ...
  function winningCondition(gameObject) {
    if (gameObject.hangman.isComplete()) {
      ...
      gameObject.wordToGuess.key.forEach(function(letter) {
        $('#word-container').append("<p class='letter red'>" + letter +
          "</p>");
      });
      alert("You lose!");
      ...
      $('#word-container').empty();
      gameObject.wordToGuess.key.forEach(function(letter) {
        $('#word-container').append("<p class='letter green'>" + letter +
          "</p>");
      });
      alert("You win!");
      $("button#play-again").fadeIn();
  ...
  $("#play-again").click(function() {
    ...
    $("#hangman-sprite").delay(400).removeClass().addClass(
      "hangman-sprite0");
  });
});
